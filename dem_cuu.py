...
from ve_vung import detecting_area, draw_bounds
...
class Counting_LiveStocks:
    ...
    def plot_boxes(self, results, frame):
        ...
        for r in results:
            ...
            frame = cv.putText(frame, 'So luong hien tai:'+str(in_sight_count), self.org1, self.font,
                    self.fontScale, self.color, self.thickness, cv.LINE_AA)
            frame = cv.putText(frame, 'Tong so:'+str(len(self.id_color.keys())), self.org2, self.font,
                    self.fontScale, self.color, self.thickness, cv.LINE_AA) 
        
        cv.imshow("Counting Sheep", frame)
        return frame
...
model_name = "best.pt"
video_path = "videos/phan_loai_cuu.mp4"
#video_path = "videos/cuu2.mp4"
